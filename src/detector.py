...
class ElephantDetector:
    ...
    def _select_device(device: Optional[str]) -> str:
        """Select execution device ('cuda', 'cpu', or specific cuda index)."""
    ...
    def _load_model(self) -> RTDETR:
        ...
        if not self.model_path.exists():
            # If standard weight is requested but not present in local weights dir, check root or allow Ultralytics to resolve
