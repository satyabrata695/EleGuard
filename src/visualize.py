...
class Visualizer:
    ...
    # Harmonious color palette for classes
    CLASS_COLORS = [
    ...
    def draw_detection(self, frame: np.ndarray, detection: Detection) -> None:
        ...
        if label:
            ...
            # Text inside badge (black text for high contrast on bright color badges)
            cv2.putText(
                frame,
                label,
    ...
    def draw_alert_banner(self, frame: np.ndarray, count: int) -> None:
        ...
        # Alert banner overlay on top right / center
        bx1 = w - tw - 30
        by1 = 10
        bx2 = w - 10
        by2 = 20 + th + baseline
    ...
    def render(self, frame_info: FrameInfo) -> np.ndarray:
        ...
        frame_info : FrameInfo
            Processed frame object containing detections and telemetry.
